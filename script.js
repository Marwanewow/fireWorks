...
let duration = 5000;
let str = []; // Starts empty until the user submits their name
...
// Create input field and button for name entry
function createInputForm() {
    ...
    form.innerHTML = `
        <input id="nameInput" type="text" placeholder="Enter your name" style="padding: 10px; font-size: 16px;" />
        <button id="submitName" style="padding: 10px; font-size: 16px; margin-left: 5px;">Start</button>
    `;
    ...
    document.getElementById('submitName').addEventListener('click', () => {
        ...
        if (name) {
            str = ['ILOVEYOU', name.toUpperCase()]; // Set the name dynamically
            form.remove(); // Remove the input form
            animationStarted = true; // Mark animation as ready to start
...
function init() {
    createInputForm(); // Add input form for name entry
...
function render(t) {
    if (!animationStarted) return; // Stop rendering if the animation hasn't started
    makeChars(t);
